...
class Task2:
    ...
    def __init__(self):
        ...
        rospy.Service("exploration/enable", SetBool, self.exploration_switch_cb)
        ...
        self.ctrl_c = False
        self.exploration_enable = False # default exploration

        rospy.on_shutdown(self.shutdownhook)
        rospy.loginfo(f"the {node_name} node has been initialised...")

    def exploration_switch_cb(self, req):  #service server to enable/disable exploration
        self.exploration_enable = req.request_signal
        print("Exploration Called: ", self.exploration_enable)
        resp = SetBoolResponse()
        resp.response_signal = True
        if self.exploration_enable:
            resp.response_message = "Exploration Started"
        else:
            resp.response_message = "Exploration Stoped"
        return resp
